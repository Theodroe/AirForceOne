...
_DEFAULT_CSV = _os.path.join(_PROJECT_ROOT, "data", "날씨", "체감온도(2020~2025, 시간별AOS).csv")
_FALLBACK_CSV = _os.path.join(_PROJECT_ROOT, "data", "heatmap_actual.csv")
CSV_PATH = _DEFAULT_CSV if _os.path.exists(_DEFAULT_CSV) else _FALLBACK_CSV
...
def load_data(path: str = CSV_PATH) -> pd.DataFrame:
    df = pd.read_csv(
        path, encoding="cp949",
        usecols=["year", "month", "day", "hour", "region", "sploc", "ta", "ws", "hm", "WCT", "rn", "dsnw"],
    )
