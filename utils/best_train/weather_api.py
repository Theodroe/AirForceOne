...
def _fetch_area(
    ...
    try:
        ...
        if result_code and result_code.text != '00':
            ...
            return [], f"{name}: {msg.text if msg else 'API 오류'}"
        ...
        for item in items:
            f_date = item.find('fcstDate').text
            if f_date not in target_dates:
                continue
            f_time = item.find('fcstTime').text[:2]
            cat    = item.find('category').text
            val_tag = item.find('fcstValue')
            if not val_tag:
                continue
            ...
            try:
                temp_dict[key][col_name] = float(val_tag.text)
            except ValueError:
                pass
