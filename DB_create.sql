...
--   3) asos_history
-- 추가:
--   4) auth_session (쿠키/토큰 기반 로그인 세션 복원용)
-- =========================================================
...
CREATE TABLE IF NOT EXISTS auth_session (
    ...
    PRIMARY KEY (token),
    INDEX idx_auth_session_user (user_id, created_at),
    INDEX idx_auth_session_sn (service_number, created_at),
    INDEX idx_auth_session_exp (expires_at),
    CONSTRAINT fk_auth_session_user FOREIGN KEY (user_id) REFERENCES users(user_id)
) ENGINE=InnoDB
  DEFAULT CHARSET=utf8mb4
  COLLATE=utf8mb4_unicode_ci
  COMMENT='로그인 세션 복원용 토큰';
